<template>
      ...
      <div class="form-structor">
        <div class="signup" :class="pageState.isLoginPage ? 'slide-up' : ''">
          ...
          <div class="form-holder">
            <input
              type="text"
              v-model="registerPage.name"
              class="input"
              placeholder="姓名"
            />
            <input
              type="text"
              v-model="registerPage.account"
              class="input"
              placeholder="账号"
            />
            <input
            ...
            <input
              type="text"
              v-model="registerPage.phone"
              class="input"
              placeholder="手机号"
            />
          </div>
          <button class="submit-btn" @click="doSignUp">注册</button>
        </div>
        <div class="login" :class="pageState.isLoginPage ? '' : 'slide-up'">
          <div class="center">
            ...
            <h4 style="text-align: center">会展展位预约管理系统</h4>
            <div class="form-holder">
              <input
                type="text"
                v-model="loginPage.account"
                class="input"
                placeholder="账号"
              />
              <input
  ...
  export default {
    ...
    methods: {
      doLogin() {
        this.$verify.emptyCheckObject(this.loginPage).then(res => {
            this.$http.userRequest.doLogin(res.account,res.password).then(result => {
                if (result.code === 200) {
                    this.$hint.message.success('欢迎你，【' + result.data.e2.name + '】');
                    let user = result.data.e2;
                    let token = result.data.e1;
                    this.$store.commit('setCurrentUser',user);
                    this.$store.commit('setToken',token);
                    if (user.identity === '参展商') {
                      this.$router.replace('/businessIndex');
                    }else {
      ...
      doSignUp() {
        this.$verify.emptyCheckObject(this.registerPage).then(obj => {
            this.$http.userRequest.doSignUp(obj).then(res => {
                if (res.code === 200) {
                    this.$hint.message.success('注册成功');
  ...
  .form-structor::after {
    ...
    background-image: url("http://programs.ptcc9.top/sos-bus/images/background_top.jpg");
    opacity: 0.8;
  }
