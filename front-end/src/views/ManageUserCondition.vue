        ...
        <el-dialog :visible.sync="isCreateDialogShow" width="45%" center>
            ...
            <el-form :model="createObj" status-icon label-width="150px" class="demo-ruleForm" style="font-size: 10px">
                ...
                <el-form-item label="身份 :">
                    <el-select style="width: 60%" clearable v-model="createObj.identity"  placeholder="身份">
        ...
        <div class="container">
            ...
            <div class="table">
                <el-table :data="tableList" style="width: 95%" height="100%" max-height="100%" empty-text="无匹配行"
                    ...
                    <el-table-column label="手机号" min-width="10" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ scope.row.mobile }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="密码" min-width="10" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ scope.row.password }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="身份" min-width="10" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
...
export default {
    ...
    methods: {
        getTableData() {
            this.$http.viewUserTable(this.searchCondition.username).then(res => {
        ...
        handleCreate() {
            if (this.dialogType === '新增') {
                ...
                this.$http.updateUser(this.createObj).then(res => {
                    if (res.code === 200) {
                        this.isCreateDialogShow = false;
                        this.getTableData();
                    }
