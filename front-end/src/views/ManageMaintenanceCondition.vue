        ...
        <el-dialog :visible.sync="isCreateDialogShow" width="45%" center>
            ...
            <el-form :model="createObj" status-icon label-width="150px" class="demo-ruleForm" style="font-size: 10px">
                ...
                <el-form-item label="联系方式 :">
                    <el-input clearable v-model="createObj.mobile" style="width: 85%"></el-input>
                </el-form-item>
                <el-form-item label="地址 :">
                    <el-input clearable v-model="createObj.address" style="width: 85%"></el-input>
                </el-form-item>
                <el-form-item label="邮箱 :">
                    <el-input clearable v-model="createObj.mailbox" style="width: 85%"></el-input>
                </el-form-item>
            </el-form>
        ...
        <div class="container">
            <div class="search">
                <el-row :gutter="20" style="margin-top: 20px;">
                    ...
                    <el-col :span="3" :offset="2">
                        <el-input clearable size="mini" placeholder="姓名" suffix-icon="el-icon-search"
                            v-model="searchCondition.username" @change="getTableData"></el-input>
                    </el-col>
                    <el-col :span="3" :offset="0">
                        <el-input clearable size="mini" placeholder="联系方式" suffix-icon="el-icon-search"
                            v-model="searchCondition.mobile" @change="getTableData"></el-input>
                    </el-col>
                    <el-col :span="3" :offset="0">
                        <el-input clearable size="mini" placeholder="邮箱" suffix-icon="el-icon-search"
                            v-model="searchCondition.mailbox" @change="getTableData"></el-input>
                    </el-col>
            ...
            <div class="table">
                <el-table :data="tableList" style="width: 95%" height="100%" max-height="100%" empty-text="无匹配行"
                    ...
                    <el-table-column label="姓名" min-width="10" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ scope.row.username }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="联系方式" min-width="10" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ scope.row.mobile }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="地址" min-width="10" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ scope.row.address }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="邮箱" min-width="10" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
...
export default {
    ...
    methods: {
        getTableData() {
            this.$http.viewMaintenanceTable(this.searchCondition.username,this.searchCondition.mobile,this.searchCondition.mailbox).then(res => {
        ...
        handleDelete(row){
            ...
            this.$hint.confirm(message).then(res => {
                if (res) {
                    this.$http.deleteMaintenance(row.id).then(res => {
                        if (res.code === 200) {
                            this.getTableData();
                        }
                    })
        ...
        handleCreate() {
            if (this.dialogType === '新增') {
                this.$http.createMaintenance(this.createObj).then(res => {
                    if (res.code === 200) {
                ...
                this.$http.updateMaintenance(this.createObj).then(res => {
                    if (res.code === 200) {
                        this.isCreateDialogShow = false;
                        this.getTableData();
                    }
