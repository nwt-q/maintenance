<template>
    <div class="main">
        ...
        <el-dialog :visible.sync="isUploadDialogShow" width="45%" center>
            ...
            <el-form :model="uploadObj" status-icon label-width="150px" class="demo-ruleForm" style="font-size: 10px">
                <el-form-item label="手册名称 :">
                    <el-input clearable v-model="uploadObj.filename" style="width: 85%"></el-input>
                </el-form-item>
                <el-form-item label="描述 :">
                    <el-input type="textarea" :rows="3" clearable v-model="uploadObj.description"
                        style="width: 85%"></el-input>
                </el-form-item>
                <el-form-item label="文件 :">
                    <el-upload class="upload-demo" action="#" :http-request="uploadFile" :on-preview="handlePreview"
            ...
            <div style="display: flex;align-items: center;">
                <el-button style="margin: auto; margin-top: 2vh; width: 50%" type="primary"
                    @click="handleUpload">确认提交</el-button>
            </div>
        </el-dialog>
        ...
        <div class="container">
                ...
                <el-row :gutter="20" style="margin-top: 20px;">
                    <el-col :span="2" :offset="1">
                        <el-button size="small" style="width: 100px" type="primary" icon="el-icon-circle-plus-outline"
                            @click="() => { this.isUploadDialogShow = true; this.dialogType = '上传文件'; this.fileList = []; this.uploadObj = {}; }">增加</el-button>
                    </el-col>

                    <el-col :span="3" :offset="1">
                        <el-date-picker v-model="searchCondition.startTime" type="datetime" size="mini"
                            placeholder="开始时间" value-format="timestamp" @change="getTableData">
                        </el-date-picker>
                    </el-col>
                    ...
                    <el-col :span="3">
                        <el-input clearable size="mini" placeholder="名称" suffix-icon="el-icon-search"
            ...
            <div class="table">
                <el-table :data="tableList" style="width: 95%" height="100%" max-height="100%" empty-text="无匹配行"
                    ...
                    <el-table-column label="手册名称" min-width="10" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ scope.row.filename }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="描述" min-width="15" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ scope.row.description }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="下载量" min-width="5" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                    ...
                    <el-table-column label="操作" min-width="15">
                        <template slot-scope="scope">
                            <el-button size="mini" type="success" @click="handleDownload(scope.row)"
...
export default {
    ...
    methods: {
        getTableData() {
            this.$http.getKnowledgeBaseList({
        ...
        handleUpload() {
            ...
            if (this.dialogType === '上传文件') {
                // 调用上传API
                this.$http.uploadKnowledgeBase(formData).then(res => {
                    if (res.code === 200) {
                ...
                this.$http.updateKnowledgeBase(formData).then(res => {
                    if (res.code === 200) {
        ...
        handleDelete(row) {
            ...
            this.$hint.confirm(message).then(res => {
                if (res) {
                    this.$http.deleteKnowledgeBase(row.filename).then(res => {
                        if (res.code === 200) {
        ...
        handleDownload(row) {
            // 下载文件访问链接
            window.location.href = ("http://127.0.0.1:18082/downloadPdf?path=" + row.filename);
            this.getTableData();
        },
